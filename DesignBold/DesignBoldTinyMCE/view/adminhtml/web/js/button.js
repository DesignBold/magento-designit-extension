...
window.DBSDK_Cfg = {
    ...
    export_callback: function (resultUrl, document_id, options) {
        ...
        var url = "http://magento.test:8080/rest/V1/designbold-tinymce/upload_image";

        DBSDK.uploadImage(url, resultUrl, "POST");
    },
};
...
(function (doc, win) {
    ...
    DBSDK.startOverlay = function (postId = null) {
        ...
        if (overlay) {
            ...
            var favorite = '<div class="fix-favorite" data-id="' + postId + '">'
                + '<div class="db-loading">'
                + '<div class="inner-circles-loader large loading-icon"></div>'
                + '</div>'
                + '<span class="db-close-lightbox">x</span>'
